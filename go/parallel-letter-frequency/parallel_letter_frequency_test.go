...
func TestConcurrentFrequencyWithSimpleStrings(t *testing.T) {
	s := []string{`a`, `b`, `c`}
	seq := OriginalFrequency(`a` + `b` + `c`)
	con := ConcurrentFrequency(s)
...
func TestConcurrentFrequencyWithSameWords(t *testing.T) {
	s := []string{`aaaaa`, `aaaaa`, `aaaaa`}
	seq := OriginalFrequency(`aaaaa` + `aaaaa` + `aaaaa`)
	con := ConcurrentFrequency(s)
